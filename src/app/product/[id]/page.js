...
import { ShoppingBagIcon as ShoppingBagIconOutline, PhotoIcon } from '@heroicons/react/24/outline';
...
const AnalysisButton = ({ productId, analysisType, label, setAnalysisResult, setIsLoadingAnalysis }) => {
  const handleClick = async () => {
    ...
    try {
      ...
      const data = JSON.parse(responseText); // Now parse JSON
  ...
  return (<button onClick={handleClick} className="px-4 py-2 rounded-lg bg-gray-800 text-white text-sm font-semibold hover:bg-gray-700 transition-colors duration-150 ease-in-out shadow-md focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-opacity-50"> {label} </button>);
};
  ...
  useEffect(() => {
    if (id) {
      const fetchProductDetails = async () => {
        console.log(`PRODUCT_DETAIL_PAGE (${id}): Fetching details...`);
        setIsLoading(true);
        setError(null);
        setMainImage(null);
        setProduct(null);
        setAnalysisResult(null); // Clear previous analysis results when loading new product
        setIsLoadingAnalysis(false); // Reset analysis loading state
        try {
          ...
          console.log(`PRODUCT_DETAIL_PAGE (${id}): Data received from API:`, JSON.stringify(data, null, 2));
          setProduct(data);
          setMainImage(data.detailImage || data.image || null);
        } catch (err) {
          console.error(`PRODUCT_DETAIL_PAGE (${id}): Error fetching product details:`, err);
      ...
      console.log("PRODUCT_DETAIL_PAGE: No ID found in params, initial load or bad URL.");
      setIsLoading(false);
    ...
    <div className="min-h-screen bg-white text-gray-900">
      ...
      <div className="container mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12 px-4 sm:px-6 md:px-8 py-10">
          {/* IMAGE BLOCK */}
          <div className="md:sticky md:top-24 h-fit"> {/* Sticky for desktop image column */}
            <div className="relative w-full aspect-[4/5] rounded-xl shadow-2xl overflow-hidden bg-gray-100">
              {imageSrcToDisplay ? (
                <Image
                  ...
                  fill
                  className="object-contain" // Changed to contain to see more of the image
                  priority
                  sizes="(max-width: 768px) 90vw, 45vw"
                  onError={() => {
                    console.warn(`PRODUCT_DETAIL_PAGE: Main Image failed to load: ${imageSrcToDisplay}. Setting placeholder.`);
            ...
            {(product.image || (product.thumbnails && product.thumbnails.length > 0)) && (
              <div className="mt-4 flex justify-center gap-2 sm:gap-3">
                {product.image && (
                  <div className="relative w-20 h-20 sm:w-24 sm:h-24 cursor-pointer rounded-md overflow-hidden border-2 hover:border-indigo-500 transition-all"
                    onClick={() => product.image && setMainImage(product.image)} >
                    <Image
                      src={product.image}
                      alt={`${product.name} main thumbnail`}
                      fill
                      className={`object-cover ${mainImage === product.image ? 'ring-2 ring-indigo-500 ring-offset-2' : ''}`}
                      onError={(e) => { e.currentTarget.parentElement.style.display = 'none'; }}
                    />
                  </div>
                )}
                {product.thumbnails && product.thumbnails.map((thumbSrc, i) => (
                  thumbSrc && (
                    <div className="relative w-20 h-20 sm:w-24 sm:h-24 cursor-pointer rounded-md overflow-hidden border-2 hover:border-indigo-500 transition-all"
                      key={i}
                      ...
                      <Image
                        ...
                        alt={`Thumb ${i + 1}`}
                        fill
                        className={`object-cover ${mainImage === thumbSrc ? 'ring-2 ring-indigo-500 ring-offset-2' : ''}`}
                        onError={(e) => { e.currentTarget.parentElement.style.display = 'none'; }}
                      />
                    </div>
                  )
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="flex items-center gap-3">
              <span className="text-yellow-500 font-bold text-xl flex items-center">
                {/* You can render stars here based on product.rating */}
                {product.rating || 0} ★
              </span>
                  ...
                  {product.sizes.map((size) => (
                    <button
                      ...
                      ${selectedSize === size
                          ...
                          : 'bg-white text-gray-700 hover:bg-gray-100 focus:ring-2 focus:ring-indigo-500'
                        }`}
                    >
                      {size}
                    </button>
            ...
            <button
              ...
              className="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-3.5 rounded-lg font-semibold text-lg transition duration-150 ease-in-out transform hover:scale-105 flex items-center justify-center gap-2 shadow-md focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-opacity-50"
            >
              <ShoppingBagIconOutline className="h-6 w-6" /> Add to Cart
            </button>
            {addedToCartMessage && (
              <div className="mt-2 text-center text-green-600 font-medium flex items-center justify-center gap-1.5 py-2 px-3 bg-green-50 rounded-md border border-green-200">
                <CheckCircleIcon className="h-5 w-5" /> {addedToCartMessage}
              </div>
            )}
            ...
            {product.shipping && product.shipping.length > 0 && product.shipping[0].label && (
              <div className='border border-gray-200 rounded-lg p-6 bg-gray-50'>
                ...
                <ul className="space-y-1.5 text-sm text-gray-600">
                  {product.shipping.map((s, i) => (<li key={i}> <strong className="text-gray-700">{s.label}:</strong> {s.value} </li>))}
                </ul>
              </div>
            )}
          </div>
      ...
      <div className="px-4 sm:px-8 md:px-20 py-10 bg-gray-100"> {/* Changed bg color */}
        <div className="container mx-auto">
          ...
          <div className="flex flex-col lg:flex-row gap-8 xl:gap-12">
              ...
              <div className="space-y-2">
                {[5, 4, 3, 2, 1].map((r) => (
                  <div key={r} className="flex items-center gap-3">
                    <span className="text-sm text-gray-600 w-12 text-right">{r} star{r > 1 ? 's' : ''}</span>
                    <div className="w-full h-2.5 bg-gray-200 rounded-full">
                      <div className="h-2.5 bg-yellow-400 rounded-full" style={{ width: getRatingPercentage(r) }} />
                    </div>
                    {/* <span className="text-xs text-gray-500 w-10 text-left">{getRatingPercentage(r)}</span> */}
                  </div>
                ))}
              </div>
            </div>
            <div className="lg:w-2/3 space-y-6">
              ...
              {isLoadingAnalysis && (
                <div className="text-center p-6 border rounded-lg bg-yellow-100 my-6 shadow-md">
                  <p className="text-lg font-semibold text-yellow-700 animate-pulse">Loading AI Analysis...</p>
                </div>
              )}

              {analysisResult && !isLoadingAnalysis && (
                <div className="p-6 border border-gray-200 rounded-xl bg-green-50 my-6 shadow-xl">
                  <h4 className="text-2xl font-bold text-green-800 mb-3 border-b border-green-200 pb-3">
                    {analysisResult.title || "AI Analysis Result"}
                  </h4>
                  ...
                  {(analysisResult.details || analysisResult.indicators || analysisResult.patterns) && (
                    <div className="mt-4">
                      <strong className="text-sm text-gray-700 block mb-1">Key Points:</strong>
                      <ul className="list-disc list-inside text-sm text-gray-600 space-y-1">
                        {analysisResult.details?.map((item, i) => <li key={`detail-${i}`}>{item}</li>)}
                        {analysisResult.indicators?.map((item, i) => <li key={`indicator-${i}`}>{item}</li>)}
                        {analysisResult.patterns?.map((item, i) => <li key={`pattern-${i}`}>{item}</li>)}
                      </ul>
                    </div>
                  )}
                  {analysisResult.breakdown && typeof analysisResult.breakdown === 'object' && (
                    <div className="mt-4 text-xs text-gray-500 border-t border-gray-200 pt-3">
                      ...
                      {Object.entries(analysisResult.breakdown).map(([key, value]) => (<p key={key}><span className="font-medium text-gray-600">{key}:</span> {String(value)}</p>))}
                    </div>
                  )}
              ...
              <div className="bg-white rounded-xl shadow-xl p-6">
                ...
                {product.reviews_list && product.reviews_list.length > 0 ? (
                      ...
                      <div key={review.review_id || i} className="border-b border-gray-200 pb-4 last:border-b-0 last:pb-0">
                        <div className="flex justify-between items-start mb-1">
                          <div>
                            ...
                            {review.verified_purchase && <span className="ml-2 text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full">Verified Purchase</span>}
                          </div>
                          <span className="text-yellow-500 font-bold flex items-center">{review.rating}★</span>
                        </div>
                  ...
                  <div className="flex flex-col items-center justify-center h-40 text-gray-500">
                    <PhotoIcon className="w-16 h-16 text-gray-300 mb-2" /> {/* Using PhotoIcon as an example */}
