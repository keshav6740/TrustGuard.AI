...
export default function ProductPage() {
  ...
  const formatCustomDateTime = (raw) => {
    if (!raw) return null;

    // Example: "15/03/23 [02.45 pm]"
    const match = raw.match(/^(\d{2})\/(\d{2})\/(\d{2})\s+\[(\d{2})\.(\d{2})\s*(am|pm)\]$/i);
    ...
    // JS Date: YYYY-MM-DDTHH:MM:00
    const isoString = `20${year}-${month}-${day}T${hour.toString().padStart(2, '0')}:${minute.toString().padStart(2, '0')}:00`;
      ...
      <div className="px-4 sm:px-8 md:px-20 py-10 bg-gray-100">
        <div className="container mx-auto">
          ...
          <div className="flex flex-col lg:flex-row gap-8 xl:gap-12">
            ...
            <div className="lg:w-1/3 bg-white rounded-xl shadow-xl p-6 space-y-6 h-fit">
                  ...
                  <div className="max-h-[500px] overflow-y-auto space-y-6 pr-2">
                    {product.reviews_list.map((review, i) => (
                      <div key={review.review_id || i} className="border-b border-gray-200 pb-4 last:border-b-0 last:pb-0">
                        ...
                        {console.log('Raw date_time:', review.date)}
                        <p className="text-xs text-gray-400 mb-2">
                          {formatCustomDateTime(review.date) || 'Date not available'}
                        </p>
